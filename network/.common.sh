...
if [ "${NO_LOCK_REQUIRED}" = "true" ];then
  if [ -f .quorumDevQuickstart.lock ];then
    echo "Network already in use (.quorumDevQuickstart.lock present)." >&2
  ...
  if [ -f .quorumDevQuickstart.lock ]; then
    ...
    composeFile=`sed '1q;d' .quorumDevQuickstart.lock`
  else
    echo "Network is not running (.quorumDevQuickstart.lock not present)." >&2
